...
options_min_arc_radius = 0.01
options_biarc_tolerance = 1
options_biarc_max_split_depth = 4
...
def cspseglength(sp1,sp2, tolerance = 0.01):
    ...
    return 0#bezmisc.bezierlength(bez, tolerance)    
...
def biarc(sp1, sp2, z1, z2, depth=0):
    def biarc_split(sp1,sp2, z1, z2, depth): 
        if depth<options_biarc_max_split_depth:
    ...
    def calculate_arc_params(P0,P1,P2):
        ...
        if (p0a<p2a and  (p1a<p0a or p2a<p1a))    or    (p2a<p1a<p0a) : 
            alpha = -2*pi+alpha 
        if abs(R.x)>1000000 or abs(R.y)>1000000  or (R-P0).mag<options_min_arc_radius**2 :
            return None, None
        else :    
            return  R, alpha
    ...
    if d > options_biarc_tolerance and depth<options_biarc_max_split_depth:
        return biarc_split(sp1, sp2, z1, z2, depth)
    else:
        ...
        l = (P0-P2).length2()
        if  l < EMC_TOLERANCE_EQUAL**2 or l<EMC_TOLERANCE_EQUAL**2 * R1.length2() /100 :
        ...
        l = (P4-P2).length2()
        if  l < EMC_TOLERANCE_EQUAL**2 or l<EMC_TOLERANCE_EQUAL**2 * R2.length2() /100 :
            # arc should be straight otherwise it could be threated as full circle
